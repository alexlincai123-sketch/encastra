...
| **ENC-NEW-16** | MEDIUM | The library caps entries (10 000) but not bytes: 10 000 imports × 64 MB is disk with no ceiling, and an automated agent driving the UI could produce it | `MAX_LIBRARY_BYTES` 4 GiB, checked before any byte is copied and refused as `ImportError::LibraryFull { max, used, needed }`; `used` measured from `imports/` rather than summed out of the index; check and copy under one index lock; staging counted and swept | `library_bytes.rs` (8: boundary, saturation, measured-vs-claimed, a refused import leaving nothing, an index that understates itself, staging counted then swept), `adaptive_attacker.rs::a_reservation_that_refuses_costs_the_library_nothing_however_often_it_is_asked`, desktop `ceiling::*` (3: two threads into room for one, rollback on a failed index write, sweep at load) | **FIXED** | The accepted overshoot is one import's true size (the check precedes the copy, bounded by `MAX_PUBLICATION_BYTES`); it is *not* an overshoot an index can widen by lying, which is what measuring buys. Disk exhaustion underneath the ceiling is an ordinary `Io` refusal, not a panic |
...
| Thousands of publications imported | Each: 64 MB, 64 entries, checksum-bound, review re-run; library ≤ 10 000 entries **and ≤ 4 GiB of copies** (ENC-NEW-16, fixed) | Every import still needs a folder chosen in the native chooser — a human gesture per import; no renderer-only path. Past the ceiling nothing is copied and the refusal says what would have to be removed |
...
4. Two LOW process findings (ENC-NEW-17, ENC-NEW-18) are carried, written down, with owners.
   The accepted MEDIUM (ENC-NEW-16, the library's byte ceiling) was fixed after this report was
   written, on `rc/import-policy`, with the tests named in its row — it is no longer carried.
