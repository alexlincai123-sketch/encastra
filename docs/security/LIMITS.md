...
* **Aggregate memory across a run.** Each file and each image is capped; the total held at once is
  not, because the executor keeps every node's output for the lifetime of the run. Twenty nodes
  each at their individual limit are twenty times the limit. Recorded as a residual risk in
  `docs/THREAT-MODEL.md` rather than fixed with a number, because fixing it properly means
  evicting a value once every consumer has read it, which the executor already knows how to work
  out and does not yet do.
