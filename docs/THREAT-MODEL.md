...
4c. **There is no aggregate memory ceiling across a run.** Each file is capped at 512 MB and each
   image at 100 megapixels, but the executor holds every node's output for the lifetime of the
   run and evicts nothing. Twenty nodes each at their individual limit are twenty times the
   limit.
...
13. **`open_project` and `restore_version` take an arbitrary path.** Reading is less dangerous
    than writing and a dialog is the intended source, but `save_project` is the only one of the
    three that constrains what it is handed.
