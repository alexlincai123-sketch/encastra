...
## What the repository carries while the decision is open (added 2026-09-15)

- `NOTICE` at the root: states that `UNLICENSED` is a decision not to grant rights yet, points
  here, and deliberately does not assert whose copyright it is — the legal identity (a person or
  a company) is part of the open decision.
