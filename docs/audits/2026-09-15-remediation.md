...
| ENC-10 | MEDIUM | **FIXED** | `read_bounded` checks `metadata` before allocating. | `the_bounded_reader_refuses_before_it_allocates` (rewritten — it did not test the refusal), `the_public_reader_is_the_bounded_one_at_the_documented_ceiling` | Aggregate memory now follows the graph's width (ENC-NEW-05, fixed). |
...
| ENC-12 | LOW | **FIXED** | `save_project` requires a `.encastra` destination. | `a_project_is_saved_only_as_a_project` | `open_project`, `restore_version`, `compare_versions` and the publication commands now share the same guard (`feat/readiness`). |
...
| **ENC-NEW-05** | MEDIUM | **FIXED** (on `feat/readiness`, merged here) | A produced value is released the moment its last consumer has finished, whatever happened to that consumer, so a run holds what the widest point of the graph needs rather than everything it ever produced. Measured on a 20 000-step chain passing a 45 KB document along: 3.9 GB → 67 MB peak; 10 000 steps 1.97 GB → 39 MB. The same change indexes each node's incoming edges once, which took the executor from quadratic to linear in the graph's size (20 000 steps: 58 s → 17 s). | `a_value_is_released_once_its_last_consumer_has_finished` (`crates/encastra-builtins/tests/end_to_end.rs`) | Width is still unbounded: twenty nodes that all feed one consumer are twenty values held at once. That is the shape a person drew, and the per-value caps still apply to each. |
...
3. **Aggregate memory across a run follows the graph's width, not its length** (ENC-NEW-05,
...
7. **`open_project` and `restore_version` took an arbitrary path** to read. Closed on
   `feat/readiness`: every command taking a project path goes through one `.encastra` guard.
